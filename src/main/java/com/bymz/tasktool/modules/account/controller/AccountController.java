...
import com.bymz.tasktool.modules.rsa.TestRSA;
...
public class AccountController {
    ...
    public String register(Account account, ModelMap modelMap){
        modelMap.put("rsaPublicKey", TestRSA.publicKey);
...
//        //注册成功跳转到首页   又或是跳转到登入页
        ...
        //用RSA解密AES的密钥
        try{
            key = TestRSA.decrypt(key, TestRSA.getPrivateKey(TestRSA.privateKey));
        }catch (Exception e){
...
//        service.save(account);
    ...
    @ResponseBody
    public Map<String, Object> doRegister2(Account account, String aesKey, ModelMap modelMap){
        //用RSA解密AES的密钥
        try{
            aesKey = TestRSA.decrypt(aesKey, TestRSA.getPrivateKey(TestRSA.privateKey));
        }catch (Exception e){
